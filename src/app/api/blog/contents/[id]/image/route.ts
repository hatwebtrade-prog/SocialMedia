...
export async function POST(request: Request, { params }: Ctx) {
  ...
  const result = await run({ contentId: id, slideIndex: null, productId: parsed.data.productId, useMockup: parsed.data.useMockup, provider: parsed.data.provider, brief: parsed.data.brief, styleId: parsed.data.styleId, blogBanner: true }, deps);
